...
contract ModredIP is ERC721, Ownable, ReentrancyGuard {
    ...
    // Chainlink CRE: trusted proxy that can call registerIPFor and mintLicenseByProxy
    address public creProxy;
    ...
    /**
     * @dev Register a new IP asset on behalf of a beneficiary (Chainlink CRE workflow).
     * Only callable by the configured CRE proxy after the Forwarder validates the report.
     */
    function registerIPFor(
        address beneficiary,
        string memory ipHash,
        string memory metadata,
        bool isEncrypted
    ) public onlyCREProxy returns (uint256) {
        ...
        ipAssets[tokenId] = IPAsset({
            ...
            registrationDate: block.timestamp,
            totalRevenue: 0,
            royaltyTokens: ROYALTY_DECIMALS
        });
        
        _mint(beneficiary, tokenId);
        emit IPRegistered(tokenId, beneficiary, ipHash);
        
        return tokenId;
    }
    
    /**
     * @dev Mint a license for an IP asset on behalf of the current IP owner (Chainlink CRE workflow).
     * Only callable by the configured CRE proxy. The licensee is specified in the report.
     */
    function mintLicenseByProxy(
        uint256 tokenId,
        address licensee,
        uint256 royaltyPercentage,
        uint256 duration,
        ...
        require(licensee != address(0), "Zero licensee");
        ...
        emit LicenseMinted(licenseId, tokenId, licensee);
        return licenseId;
    }
