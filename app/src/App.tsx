...
type ContractAddresses = {
  ...
  "ModredIPModule#ModredIPCREConsumer"?: string;
};
...
const pinFileToIPFS = async (file: File): Promise<{
  ...
  try {
    ...
    const metadata = {
      name: file.name,
      description: `Uploaded via Casamigos frontend`,
      attributes: {
        uploadedBy: 'Casamigos',
  ...
  const checkBackendStatus = async () => {
    try {
      ...
      if (!wasConnected && isConnected) {
        notifySuccess('Backend Connected', 'Successfully connected to the Casamigos backend service');
      } else if (wasConnected && !isConnected) {
        notifyError('Backend Disconnected', 'Lost connection to the Casamigos backend service');
      }
    } catch (error) {
      const wasConnected = backendStatus;
      ...
      if (wasConnected) {
        notifyError('Backend Error', 'Failed to connect to the Casamigos backend service');
      }
    }
  };
    ...
    try {
      ...
      if (result.testing) {
        ...
        notifySuccess('IP Asset Registered',
          `Successfully registered IP asset!\nTransaction: ${result.base.txHash}\nIP Asset ID: ${result.base.ipAssetId}`,
          {
            action: {
              label: 'View Transaction',
              onClick: () => window.open(`https://sepolia.basescan.org/tx/${result.base.txHash}`, '_blank')
            }
          }
        );
      }
      ...
      if (result.warning) {
        ...
        notifySuccess('License Minted',
          `Successfully minted license!\nTransaction: ${result.data.txHash}`,
          {
            action: {
              label: 'View Transaction',
              onClick: () => window.open(`https://sepolia.basescan.org/tx/${result.data.txHash}`, '_blank')
            }
          }
        );
      } else {
          ...
          <div className="landing-hero">
            ...
            <div className="landing-hero-grid">
              <div className="landing-hero-copy">
                <h2 className="landing-title">Secure, license, and monitor your IP with Casamigos</h2>
              ...
              <div className="landing-illustration">
                <svg viewBox="0 0 420 320" role="img" aria-label="Casamigos overview illustration">
                  <defs>
