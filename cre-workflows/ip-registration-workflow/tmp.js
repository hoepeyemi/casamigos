...
var onCronTrigger = (runtime2) => {
  ...
  if (!evmConfig?.consumerAddress || evmConfig.consumerAddress.startsWith("REPLACE_")) {
    runtime2.log("Skipping onchain write: set consumerAddress in config.staging.json");
    return {
  ...
  runtime2.log(`Fetching external API: ${runtime2.config.apiUrl}`);
  const apiStatus = runtime2.runInNodeMode((nodeRuntime) => {
  ...
  runtime2.log(`External API response status: ${apiStatus}`);
  const demo = runtime2.config.demoRegistration;
  ...
  const writeResult1 = evmClient.writeReport(runtime2, {
  ...
  if (license && tokenIdForLicense != null) {
    ...
    const licenseReportData = encodeAbiParameters(parseAbiParameters("uint8 instructionType, bytes params"), [INSTRUCTION_MINT_LICENSE, licenseParams]);
    const reportResponse2 = runtime2.report({
      encodedPayload: hexToBase64(licenseReportData),
      encoderName: "evm",
      signingAlgo: "ecdsa",
      hashingAlgo: "keccak256"
    }).result();
    const writeResult2 = evmClient.writeReport(runtime2, {
      receiver: evmConfig.consumerAddress,
