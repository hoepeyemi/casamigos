...
## Yakoa registration and infringement check

After **registerIP**, the script registers the new IP asset with Yakoa (via `backend/src/scripts/register-ip-to-yakoa.ts`) so that the asset is known to Yakoa for infringement monitoring. At the end of each run it then invokes the backend infringement check for **all IP assets** (read token IDs from the contract, then call Yakoa for each). Ensure **backend/.env** has `YAKOA_API_KEY`, `YAKOA_SUBDOMAIN`, and `YAKOA_NETWORK`. To skip both Yakoa registration and the infringement check, set `RUN_INFRINGEMENT_CHECK=false`. You can also run the check alone from the backend: `cd backend && npm run check:infringements`.
